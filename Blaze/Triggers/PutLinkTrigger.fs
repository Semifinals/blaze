...
module PostLinkTrigger =
    ...
    [<Response(HttpStatusCode.OK, Summary = "Successful request, already exists", Description = "Indicates the request was successful but that the existing value is identical")>]
    [<Response(HttpStatusCode.Created, Summary = "Successful request, created or updated", Description = "Indicates the short URL was successfully created or updated")>]
    let Run
